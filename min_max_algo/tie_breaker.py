...
path_to_testcases = "sample test cases/"

inputs = open(path_to_testcases + "input8.txt", "r")
input_data = []
i = 0
for line in inputs:
        ...
        if curr_player[i][0] == sec_last:
            ...
            for count in range(0, 2):
                ...
                if curr_state[curr_row + row_incr][curr_col - a][0].rstrip() == myself:
                    no_plyr = (
                        int(curr_state[curr_row + row_incr][curr_col - a][1].rstrip())
                        + 1
                    )
                    node_id = create_node(
                        curr_state,
                        curr_player[i],
                        [curr_row + row_incr, curr_col - a],
                        parent,
                    )
                    action_nodes.append(node_id)
                    Nodes[str(node_id)].state[curr_row + row_incr][
                        curr_col - a
                    ] = myself + str(no_plyr)
                ...
                if ((curr_col - a) >= 0) and ((curr_col - a) <= 7):
                    ...
                    if curr_state[curr_row + row_incr][curr_col - a][0].rstrip() == opp:
                        if (curr_col - a * 2 < 0) or (curr_col - a * 2 > 7):
                            ...
                            continue
                        if (
                            curr_state[curr_row + row_incr * 2][
                                curr_col - a * 2
                            ].rstrip()
                            == "0"
                        ):
                            node_id = create_node(
                                curr_state,
                                curr_player[i],
                                [curr_row + row_incr * 2, curr_col - a * 2],
                                parent,
                            )
                            action_nodes.append(node_id)
                            Nodes[str(node_id)].state[curr_row + row_incr][
                        ...
                        if (
                            curr_state[curr_row + row_incr * 2][curr_col - a * 2][
                                0
                            ].rstrip()
                            == myself
                        ):
                            ...
                            action_nodes.append(node_id)
                            Nodes[str(node_id)].state[curr_row + row_incr * 2][
                                curr_col - a * 2
                            ] = myself + str(no_plyr)
                            Nodes[str(node_id)].state[curr_row + row_incr][
                                curr_col - a
...
def is_terminal(node_id, player):
    ...
    if not actions:
        ...
        if (
            Nodes[str(Nodes[str(node_id)].parent)].pass_status == True
            and Nodes[str(node_id)].pass_status == True
        ):
...
def minimax(input_state):
    ...
    if Nodes[str(0)].eog == True:
        nxt_action = "pass"
        print("Our Action to be taken should be: ", nxt_action)
        ...
        for i in range(len(frst_children)):
            if Nodes[str(frst_children[i])].utility == utility:
                ...
                action = Nodes[str(nxt_action)].state
                print("\n\n\n")
                print("Our Action to be taken should be: ", nxt_action)
                for i in range(len(action)):
                    print(action[i])
                print(
                    "Farsighted Utility of minimax Action: ",
                    Nodes[str(nxt_action)].utility,
                )
                farsighted = Nodes[str(nxt_action)].utility
                print("Myopic Utility of minimax Action: ", get_utility(nxt_action))
                print("\n\n")
                write_to_file(
                    nxt_action, get_utility(nxt_action), farsighted, len(Nodes)
                )
                break
        ...
        for i in range(len(frst_children)):
            if Nodes[str(frst_children[i])].utility == utility:
                ...
                for i in range(len(action)):
                    print(action[i])
                farsighted = Nodes[str(nxt_action)].utility
                print("Farsighted Utility of Action: ", Nodes[str(nxt_action)].utility)
                print("Myopic Utility of Action: ", get_utility(nxt_action))
                print("\n\n")
                write_to_file(
                    nxt_action, get_utility(nxt_action), farsighted, explored_nodes
                )
                break
    # Remember to put a checker for a Pass game.
...
def write_to_file(node, myopic, farsighted_utility, no_nodes):
    ...
    if node == "pass":
        ...
        init_sq = str(row_alphabets[Nodes[str(node)].init_pos[0]]) + str(
            Nodes[str(node)].init_pos[1] + 1
        )
        final_sq = str(row_alphabets[Nodes[str(node)].final_pos[0]]) + str(
            Nodes[str(node)].final_pos[1] + 1
        )
...
def print_state(node_id):
    print("Node being printed is: ", node_id)
    action = Nodes[str(node_id)].state
    for i in range(len(action)):
        print(action[i])
    return
