...
props = InProp()
props.acts = ["Walk Up","Walk Down","Walk Left","Walk Right",
              "Run Up","Run Down","Run Left","Run Right","None","Exit"]

props.neighbors = np.array( [ [1,0],
                             [2,0],
                             [-1,0],
                             [-2,0],
                             [0,-1],
...
for i in range(2,2+props.n_walls):
    curr_loc = [ int(input_data[i].rstrip().split(",")[0])-1,
...
props.term_nodes = [props.term_nodes for rewards, props.term_nodes
                    in sorted(zip(rewards,props.term_nodes))]
props.p_walk = float(input_data[2+props.n_walls+1+props.n_term].rstrip().split(",")[0])
props.p_run = float(input_data[2+props.n_walls+1+props.n_term].rstrip().split(",")[1])
props.r_walk = float(input_data[2+props.n_walls+1+props.n_term+1].rstrip().split(",")[0])
props.r_run = float(input_data[2+props.n_walls+1+props.n_term+1].rstrip().split(",")[1])
...
for i in range(props.n_rows):
    ...
    for j in range(props.n_cols):
        ...
        for k in [1,3,5,7]:
            nxt_row = i + props.neighbors[k][0]
            nxt_col = j + props.neighbors[k][1]
            if not boundary_viol(nxt_row, nxt_col, props) and run_validity(i, j, nxt_row, nxt_col, props):
                actions.append([nxt_row, nxt_col])
            else:
                actions.append([i,j])
        row.append(actions)
